...
const CustomHeader = ({ type }) => {
  ...
  const handleSearch = async () => {
    ...
    if (query.length > 0) {
      if (type == "Movies") {
        result = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=1&include_adult=false&query=${query}`)
